...
namespace Ax3.IMS.DataAccess.Mongo
{
    public class Repository<T> : IRepository<T> where T : IMongoEntity
    {
        ...
        private Task<IAsyncCursor<T>> QueryAsync(FilterDefinition<T> filter)
        {
            return Collection.FindAsync(filter);
        }

        private Task<IAsyncCursor<T>> QueryAsync(Expression<Func<T, bool>> filter)
        {
            return Collection.FindAsync(filter);
        }

        private Task<IAsyncCursor<T>> QueryAsync()
        {
            return Collection.FindAsync(Filter.Empty);
        }
        ...
        /// <summary>
        /// find entities with paging async
        /// </summary>
        /// <param name="filter"></param>
        ...
        /// <summary>
        /// find entities with paging and ordering
        /// default ordering is descending async
        /// </summary>
        /// <param name="filter"></param>
        ...
        /// <summary>
        /// find entities with paging and ordering in direction async
        /// </summary>
        /// <param name="filter"></param>
        /// <param name="order"></param>
        /// <param name="pageIndex"></param>
        ...
        public virtual Task<List<T>> FindAsync(Expression<Func<T, bool>> filter, Expression<Func<T, object>> order, int pageIndex, int size, bool isDescending)
        {
            return Retry(() =>
            {
                ...
                return (isDescending ? query.SortByDescending(order) : query.SortBy(order)).ToListAsync();
            });
        }
        ...
        /// <param name="updates">updated field(s)</param>
        /// <returns>true if successful, otherwise false</returns>
        public long Update(Expression<Func<T, bool>> filter, params UpdateDefinition<T>[] updates)
        {
            return Retry(() =>
            {
                var update = Updater.Combine(updates).CurrentDate(i => i.ModifiedOn);
        ...
        /// <param name="filter">collection filter</param>
        /// <param name="updates">updated field(s)</param>
        public Task<long> UpdateAsync(Expression<Func<T, bool>> filter, params UpdateDefinition<T>[] updates)
        {
            return Retry(() =>
            {
                return Task.Run(() =>
